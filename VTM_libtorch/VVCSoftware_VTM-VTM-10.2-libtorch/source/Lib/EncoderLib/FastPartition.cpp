...
void FastPartition::init_luma_feature_maps(int w, int h, int qp, int (*output_array)[550][970][9][9][4][7],int (*chroma_output_array)[40][70][5]){
  ...
  for(int i=0;i<6;i++){
    res[i*2]=torch::jit::load("./win_pt_models/"+std::to_string(i)+"/res-0.pt"); 
    res[i*2].to(torch::kCUDA);
    res[i*2].eval();
    res[i*2+1]=torch::jit::load("./win_pt_models/"+std::to_string(i)+"/res-1.pt"); 
    res[i*2+1].to(torch::kCUDA);
    res[i*2+1].eval();
    subnet[i]=torch::jit::load("./win_pt_models/"+std::to_string(i)+"/res-2.pt");
  ...
  for(int x=0;x<h/2;x++){
      for(int y=0;y<w/2;y++){
        output_stream<<cattensors[1][2][x][y].item()<<" ";
      }
  ...
  for(int k=0;k<int(input_size/batch_size_5)+1;k++){
    ...
    for(int batch_item=0;batch_item<batch_end_idx;++batch_item){
      ...
      //printf("%d %d %d %f %f %f %f\n",frame,posh,posw,ptr[batch_item*6],ptr[batch_item*6+1],ptr[batch_item*6+2],ptr[batch_item*6+3]);
      for(int i=0;i<4;i++){  //(*output_array)[550][970][9][9][4][7]
        split_list[i]=ptr[batch_item*6+i]>threshold;
        chroma_output_array[frame][posh/32][posw/32][i]=split_list[i]; //TODO
      }
      //std::cout<<frame<<" "<<posh*2<<" "<<posw*2<<" "<<split_list[0]<<" "<<split_list[1]<<" "<<split_list[2]<<" "<<split_list[3]<<" "<<0
      //<<" "<<0<<std::endl;
      if(split_list[0]+split_list[1]+split_list[2]+split_list[3]+split_list[4]+split_list[5]==0){ //TODO
        split_list[torch::argmax(output.slice(0,batch_item,batch_item+1),1).item<int>()]=1; //见vvc相关中的vtm-lmcs
      }
  ...
  for(int k=0;k<int(input_size/batch_size_0)+1;k++){
    ...
    for(int batch_item=0;batch_item<batch_end_idx;++batch_item){
      ...
      if(split_list[0]+split_list[1]+split_list[2]+split_list[3]+split_list[4]+split_list[5]==0){
        //printf("%f %f %f %f %f %f\n",ptr[batch_item*6],ptr[batch_item*6+1],ptr[batch_item*6+2],ptr[batch_item*6+3],ptr[batch_item*6+4],ptr[batch_item*6+5]);
        //printf("%d\n",torch::argmax(output.slice(0,batch_item,batch_item+1),1).item<int>());
